...
Outro objetivo principal do projeto é a leitura de ficheiros. Estes tem uma configuração definida pelo ficheiro de configuração, depois a leitura do \textit{CSV} com a informação dos jogadores e mais tarde o geramento de um página \textit{HTML} onde é apresentada a informação tratada.

Para a implementação foi necessário a construção de gramáticas independentes de contexto, implementação de parsers para cada ficheiro. A utilização do \textit{Lex} e \textit{Yacc} foi necessária para retirar a informação relevante.
...
\textit{Flex} é uma ferramenta para gerar automaticamente analisadores léxicos, isto é, programas que reconhecem padrões léxicos num texto. O \textit{Flex} é uma evolução da ferramenta \textit{Lex}, mas com a caraterística de ser mais rápido que este (\textit{Fast Lex}).

\textit{Yacc} é utilizado para sistemas operacionais \textit{Unix}, onde o seu principal objetivo e gerar analisadores sintáticos. No entanto o \textit{Yacc} tem de ser utilizado em conjunto com o \textit{Lex}, pois o \textit{Yacc} não consegue ler a partir de uma entrada de dados, daí a utilização do \textit{Lex} para que este gere os tokens que mais tarde são usados pelo \textit{Yacc} para o processamento.
...
A utilização do \textit{Flex}, \textit{Yacc} e gramáticas independentes de contexto era necessária para o desenvolvimento da aplicação, de forma a permitir o tratamento de ficheiros específicos e para gerar o \textit{HTML}, como os resultados pretendidos.
...
Antes de ser criado o \textit{HTML} a aplicação tem de ser configurada. Para isso a informação é lida de um ficheiro de configuração estão definidos os campos:
\begin{itemize}
...
A estrutura que é gerada para o armazenamento dos campos que a gramática abstrata definiu, é divida consoante o nível em que se está. Assim, começa-se pela estrutura que vai abrangir mais campos, que neste caso corresponde ao total de configurações, depois pela configuração, e, por último, a estrutura que vai guardar um campo para uma configuração.
...
Verifica-se aqui que cada \textit{struct} acima apresentada deriva de cada uma das produções que foram indicadas na gramática abstrata.
As produções que foram usadas para definir as produções da gramática abstrata são as que também foram usadas para definir as produções do \textit{Yacc}.
\newpage
...
Foram implementandas funções de libertação de memória para garantir que toda a memória, que inicialmente foi feita, para o malloc, existe um free, garantindo assim que a aplicaçãos não tem qualquer tipo de memory leek.
...
Para o ficheiro de configuração o \textit{Yacc} apanha os campos relativos à configuração do ficheiro. Cada uma das produções trata de um caso específico relativo aos campos. Assim, genericamente existe um campo e uma lista de campos, sendo necessário adicioná-lo à lista, daí exitir \texttt{\$\$ = cons\_cfg\_Lcampos\_Lcampos(\$1,\$3); }

A informação que o \textit{Yacc} recebe vem do processamento que o flex faz do ficheiro de configuração que ao \textit{Flex} é passado.
Quando a informação do flex é recebida pelo \textit{Yacc}, este apenas tem de associar a uma das suas produções de modo a que se incluída na estrutura correta.

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%LEX-CONFIGUraÇÕES%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\textit{LEX}}
...
A informação que o ficheiro \textit{Flex} recebe é proveniente do ficheiro de configuração.
Depois de reconhecermos os campos, este passam do \textit{Flex} para o \textit{Yacc} através da estrutura que é interna no \textit{Yacc}.
Por exemplo, quando se utiliza a função \texttt{cfglval.cfg\_str}, siginfica que se está a passar (neste caso) uma string ao campo cfg\_str da union, onde posteriormente o \textit{Yacc} irá adicioná-lo à sua estrutura.
\newpage
...
\item \textbf{Carregar resultado de prova}: o sistema carrega e processa um ficheiro CSV. Como resultado desta operação produz um ficheiro \textit{HTML} com a pontuação de cada atleta, ordenado por ordem descente de pontos.
\item \textbf{Calcular ranking}: em qualquer momento poderá ser invocada esta operação à qual o sistema responder com uma listagem em \textit{HTML} do raking.
\item \textbf{Gravar base de dados}: o sistema guarda em memória o estado da aplicação.
\item \textbf{Imprimir}: que apresenta a informação através de uma página \textit{HTML}.
...
Neste caso é importante referir que quando são executadas funções \texttt{void cmd\_config(char* ficheiro)}, e \texttt{void cmd\_import(char* ficheiro);} estas recebem o parser do ficheiro que lhe está associado. Se tivermos o cmd\_config op parser que é invocado é o parser responsável pelo ficheiro de configuração e o cmd\_import "chama" o parser que é responsável pelo ficheiro CSV.

A função \texttt{void cmd\_print();} é responsável por invocar a função que vai dar origem à página \textit{HTML} pretendida.

\newpage


%%%%%%%%%%%%%%%%%%%%%%%%%%YACC-CMD%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\textit{Yacc}}

No ficheiro do \textit{Yacc} cada produção que seja "apanhada" é chamada à função correpondente para tratar da operação que foi capturada.
...
No \textit{Yacc} no caso dos comandos variarem do \textit{Yacc} do ficheiro de configuração, porque neste caso e embora estejamos a receber os dados do \textit{Flex}, e embora o tratamento que o \textit{Yacc} lhe faz é diferente, isto vem pelo facto de uma produção neste caso desencadeia uma ação, isto é, quando temos por exemplo o \textit{PRINT} temos que quando esta produção é apanhada, temos de a função \texttt{cmd\_print}, que vai imprimir a informação relativa aos jogadores para uma página \textit{HTML}.
...
O \textit{Flex} neste caso está associado ao input que é produzido pelo utilizador, isto é, no caso do cmd não estamos a trabalhar com a nossa própria consola.

Devido ao facto de termos a nossa própria consola isso implica, termos o parsing que reconhece os comandos que estão a ser introduzidos, para depois esse comandos são passados ao \textit{Yacc} que vai determinar que função é desencadeada como esse comando.
...
As produções que se podem encontrar na gramática abstrata são provenientes da definição de produções a que achamos que fossem importantes para tratar a informação que pode ser encontrada num ficheiro de resultados.

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%ESTRUTURA DE DADOS -FICHEIROS-DE-RESULTADOS-%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Estrutura de Dados da Espeficicação dos Ficheiros de Resultados}

A estrutura que é gerada para o armazenamento dos campos que a gramática abstrata definiu, é divida consoante o nível em que estamos, isto é, se começarmos por ter a estrutura que vai abragir mais campos, neste caso o total de linhas que constituem o ficheiro, depois temos o que é as linhas e por último a estrutura que vai guardar um para uma campo.
...
\subsubsection{Assinatura das funções utilizadas}
Nesta sessção serão apresentadas as assinaturas das funções que foram utilizadas para dar suporte à implementação do projecto.
...
Nesta secção é apresentadas as assinaturas das funções que são usadas, tanto para fazer a inicialização das estrutura de dados (criadas pelo gabs), quer como as funções definidas por nós próprios, de modo a ser possível efectuar operações sobre as mesmas.

Para além das funções que são responsáveis por fazer a libertação de toda a memória que foi utilizada, temos a função \texttt{Linha csv\_Linha\_reverse( Linha l );} que é utilizada quando é necessário imprimir a forma como os campos foram lidos. Esta função é necessária devido ao facto de se ter definido uma gramática com recursividade à esquerda.

\newpage

\subsubsection{YACC}
%%%%%%%%%%%%%%%%%%%%%%%%%%YACC-FICHEIROS-DE-RESULTADOS-%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
No ficheiro do \textit{YACC} cada produção que seja "apanhada" é chamada a função correspondente para tratar o caso apanhado.
...
A informação que o \textit{Yacc} recebe vem do processamento que o \textit{Flex} faz do ficheiro de configuração que é passado ao \textit{Flex}.
Quando a informação do \textit{Flex} é recebida pelo \textit{Yacc}, este apenas tem de a associar a uma das suas produções de modo a ser incluída na estrutura correta.
...
O \textit{Flex} relativo ao parsing do fihceiro CSV é relativamente simples, isto deve-se ao facto de se ter definido que todos os campos do ficheiro são strings, assim sendo o único tratamento que é necessário fazer aqui e o de ver onde acaba cada string, o tratamento dos \\n, a forma de separação de cada campo e por fim o EOF.

Com esta implementação temos que é o \textit{Yacc} que decide o modo como se irá tratar cada campo que recebe do \textit{Lex}.
...
Uma vez que exitem vários atletas foi necessário a criação de uma estrutura que nos permitesse guardar a informação de cada uma dos atletas que participam na Orientação.

Essa estrutura é então composto por:
\begin{itemize}
\item \textit{ID}: identificador de atleta;
\item \textit{nome}: nome do atleta;
\item \textit{score}: lista de ponto obtidos em cada prova;
\item \textit{tempo\_ultima\_prova}: tempo que o atleta obteve na ultima prova decorrida.
\end{itemize}

Para além de termos a informação do atleta foi necessário implementar também uma estrutura que nos premitisse guardar todos os atletas que participam numa prova.

Assim sendo as estrturas implementadas foram as seguintes:
...
O tratamento desta informação serve para se poder analisar e processar os rakings das pontuações dos atletas. Deste modo, um dos requisitos necessários para processar foi os tempos obtidos por cada jogador de forma a que mais tarde, fosse possível criar no \textit{HTML} rankings de provas, atletas, etc.

\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%HTML%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{HTML}

Um dos resultados finais deste projeto é gerar uma página \textit{HTML} que permita evidenciar o raking das provas e é mais uma forma de apresentação do resulatdos obtidos.
...
Como a criação de um ficheiro \textit{HTML} era necessário ter um cuidado especial e foi preciso definir o \textit{HTMl} por partes.
Assim, a apresentação do \textit{HTML} parte por ter um índice para os rakings que poderá ser visível ao longo da página, seguindo, então, pelos respectivos rankings.
...
Ao longo do desenvolvimento do projeto o grupo desenvolveu competências em diversas ferramentas como \textit{C}, \textit{HTML}, \textit{Flex}, \textit{Yacc}, gramáticas independentes do contexto, que serão uma mais valia para o mercado de trabalho e para a vida profissional. 
